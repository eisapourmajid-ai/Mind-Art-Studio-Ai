# Mind-Art-Studio-Ai

Permanent intelligence architecture of the **AI Architecture Studio Director**.

This repository is a *development environment*: it builds and maintains the system's
intelligence. It does not execute architectural projects.

## Repository structure

```
system/
├── PROTOCOL/
│   ├── 00_DEVELOPMENT_DIRECTIVE.md    # Governing directive: role, protocol, processes
│   └── UPDATE_PROPOSAL_TEMPLATE.md    # Required format for every proposed change
├── _intake/
│   └── INTAKE_LOG.md                  # Verbatim buffer for incoming PARTS (temporary)
├── PROJECT_INSTRUCTIONS.md            # Master operating instruction (max 8000 chars)
├── 00_IDENTITY.md                     # WHO the studio is
├── 01_CORE.md                         # HOW the system thinks
└── 02_LIBRARY.md                      # WHAT the system has learned
```

## Language rules

- Dialogue with Majid: **Persian (Farsi)**.
- All file content: **English only**.

## Current state

The three core files exist as empty structures. Content is being dictated by Majid
item by item; each item is inserted into its target section exactly as given.
Full system analysis (classification review, contradiction detection, optimization)
runs afterwards.
